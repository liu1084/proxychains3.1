			...
			chain_type *ct)
{
	...
	char local_in_addr, local_netmask;
	FILE* file;
					...
					*ct=DYNAMIC_TYPE;
                    ...
                    PDEBUG("added localnet = %s%s\n", &local_in_addr, &local_netmask);
                      if (num_localnet_addr < MAX_LOCALNET) {
                        int error;
                        error = inet_pton(AF_INET, local_in_addr, &localnet_addr.in_addr);
                        if (error <= 0) {
                          fprintf(stderr, "localnet address error\n");
                          exit(1);
                        }
                        error = inet_pton(AF_INET, local_netmask + 1, &localnet_addr.netmask);
                        if (error <= 0) {
                          fprintf(stderr, "localnet address error\n");
                          exit(1);
                        }
...
int connect (int sock, const struct sockaddr *addr, unsigned int len)
{
    ...
    //  PDEBUG("localnet: %s; ", inet_ntop(AF_INET,
    //                                    &in_addr_localnet, str, sizeof(str)));
    // PDEBUG("netmask: %s; " , inet_ntop(AF_INET,
    //                                    &in_addr_netmask, str, sizeof(str)));
    // PDEBUG("target: %s\n", inet_ntop(AF_INET, p_addr_in, str, sizeof(str)));

      for (i = 0; i < num_localnet_addr; i++) {
        if ((localnet_addr.in_addr.s_addr & localnet_addr.netmask.s_addr) == (p_addr_in->s_addr & localnet_addr.netmask.s_addr))
        {
